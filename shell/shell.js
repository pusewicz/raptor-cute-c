...
window.Module = {
    preInit: [quantizeDevicePixelRatio],
    print(...args) {
        console.log(...args);
    },
    printErr(...args) {
    ...
    instantiateWasm(imports, callback) {
        const onProgress = (percent) => {
            if (percent <= 100) {
                // When compression is used, the real size is larger than Content-Length
        ...
        fetchAndInstantiateWasm(findWasmBinary(), onProgress, imports)
            .then(callback)
            .catch((err) => {
