...
#define ECS_GET(entity, T)                                                   \
    (T*)ecs_get(                                                             \
        g_state->ecs, entity, g_state->components.COMPONENT_TO_UNDERSCORE(T) \
    )
...
#define ECS_REGISTER_COMPONENT(T, ...)                               \
    g_state->components.COMPONENT_TO_UNDERSCORE(T) =                 \
        ecs_register_component(g_state->ecs, sizeof(T), __VA_ARGS__)
#define ECS_REGISTER_SYSTEM(name, ...)                                \
    g_state->systems.name =                                           \
        ecs_register_system(g_state->ecs, name##_system, __VA_ARGS__)
#define ECS_REQUIRE_COMPONENT_EACH(system, T)          \
    ecs_require_component(                             \
        g_state->ecs,                                  \
        g_state->systems.system,                       \
        g_state->components.COMPONENT_TO_UNDERSCORE(T) \
    );
...
#define ECS_UPDATE_SYSTEM(name)                                           \
    ecs_update_system(g_state->ecs, g_state->systems.name, CF_DELTA_TIME)
#define ECS_SET_SYSTEM_CALLBACKS(name)                                       \
    ecs_set_system_callbacks(                                                \
        g_state->ecs, g_state->systems.name, name##_system, nullptr, nullptr \
    )

// Add a component to an entity
#define ECS_ADD_COMPONENT(entity, T)                                    \
    ((T*)add_component_impl(                                            \
        entity, g_state->components.COMPONENT_TO_UNDERSCORE(T), nullptr \
    ))
