...
ecs_id_t make_player(GameState *state, float x, float y) {
  ...
  CF_V2 *pos = ecs_add(state->ecs, id, PosComp, nullptr);
  pos->x     = x;
  pos->y     = y;

  CF_V2 *vel = ecs_add(state->ecs, id, VelComp, nullptr);
  vel->x     = 0.0f;
  vel->y     = 0.0f;

  input_t *input = ecs_add(state->ecs, id, InputComp, nullptr);
  input->up      = false;
  input->down    = false;
  input->left    = false;
  input->right   = false;

  CF_Sprite *sprite = ecs_add(state->ecs, id, SpriteComp, nullptr);
  CF_Result  result;
