...
ecs_id_t make_player(GameState *state, float x, float y) {
  ...
  CF_V2 *pos = ecs_add(state->ecs, id, state->components.position, nullptr);
  pos->x     = x;
  pos->y     = y;

  CF_V2 *vel = ecs_add(state->ecs, id, state->components.velocity, nullptr);
  vel->x     = 0.0f;
  vel->y     = 0.0f;

  input_t *input = ecs_add(state->ecs, id, state->components.input, nullptr);
  input->up      = false;
  input->down    = false;
  input->left    = false;
  input->right   = false;

  CF_Sprite *sprite = ecs_add(state->ecs, id, state->components.sprite, nullptr);
  CF_Result  result;
...
ecs_id_t make_bullet(GameState *state, float x, float y, Direction direction) {
  SDL_Log("Making bullet at (%.2f, %.2f) going %s\n", x, y, DIRECTION_UP == direction ? "UP" : "DOWN");
  ecs_id_t id = ecs_create(state->ecs);

  bullet_t *bullet  = ecs_add(state->ecs, id, state->components.bullet, nullptr);
  ...
  CF_V2 *vel = ecs_add(state->ecs, id, state->components.velocity, nullptr);
  vel->x     = 0.0f;
  vel->y     = DIRECTION_UP == direction ? 2.0f : -2.0f;
